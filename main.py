...
def create_data_loaders(args):    
    ...
    images_val = list(df_val['image'].values)
    labels_val = list(df_val['label'].values)
    images.append(images_val)
    labels.append(labels_val)

    random_sample_image = random.choice([i for i in range(len(images) - 1)])
    image = Image.open(images[random_sample_image])
    depth = Image.open(labels[random_sample_image])
...
train = pd.read_csv("/home/kwon/sparse/data/custom/dataset_train.csv", names = ['image', 'label'])
val = pd.read_csv("/home/kwon/sparse/data/custom/dataset_val.csv", names = ['image', 'label'])
print(len(train))
...
if __name__ == '__main__':
    main()
    #resnet = ResNet(18, decoder='deconv3', output_size= (228,304)).cuda()
