...
train = pd.read_csv("/home/kwon/datasets/dataset_train.csv", names = ['image', 'label'])
val = pd.read_csv("/home/kwon/datasets/dataset_val.csv", names = ['image', 'label'])
train_loader = None
val_loader = None
...
def create_data_loaders(args):
    # Data loading code
    print("=> creating data loaders ...")
    train_dataset = pd.read_csv("/home/kwon/datasets/dataset_train.csv", names = ['image', 'label'])
    val_dataset = pd.read_csv("/home/kwon/datasets/dataset_val.csv", names = ['image', 'label'])
    train_loader = None
    val_loader = None
    ...
    from dataloaders.custom_dataloader import DepthDataset
    dataset_train = DepthDataset(train_dataset, type='train',
                modality=args.modality, sparsifier=sparsifier)
    
    lengths = [int(math.floor(len(dataset_train) * 0.8)), int(math.ceil(len(dataset_train) * 0.2))]
    dataset_train, val_dataset = torch.utils.data.random_split(dataset_train, lengths)
...
def main():
    ...
    # create new model
        ...
        if args.arch == 'resnet50':
            ...
            model = MobileNet(decoder=args.decoder, output_size=16)
        elif args.arch == 'mobilenetskipadd':
            model = MobileNetSkipAdd(output_size=16)
